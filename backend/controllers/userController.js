...
const followUnFollowUser = async (req, res) => {
    try {
        ...
        if (id === req.user._id.toString()) return res.status(400).json({ error: "You cannt follow/unfollow yourself" })
        if (!currentUser || !userToModify) return res.staus(400).json({ error: "User not found" })
...
export { followUnFollowUser, getSearchedUser, getSuggestedUser, getUserFollowers, getUserFollowing, getUserProfile, loginUser, logoutUser, signupUser, updateProfile };
